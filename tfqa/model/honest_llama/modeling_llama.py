...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.custom_attention_mask = None
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.custom_attention_mask is not None:
            for l in range(len(self.custom_attention_mask)):
                attn_weights[l][:][:]*=self.custom_attention_mask[l][:attn_weights.shape[-1]].to(attn_weights.device)
        ...
        self.attn_score = attn_output.detach().cpu().numpy()
        if self.config.pretraining_tp > 1:
